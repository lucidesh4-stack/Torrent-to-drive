...
def build_app_client(base: Path):
    ...
    `base` is the directory that CONTAINS the `streamly_hardened` package.
    """
    try:
        ...
        from streamly_hardened.app import create_app  # type: ignore
        app = create_app()
...
def main():
    ...
    ap.add_argument("--root", default="",
                    help="dir CONTAINING streamly_hardened/ (CI: the repo checkout). "
    ...
    # Resolve base (the dir that holds the streamly_hardened package).
    base = Path(args.root or os.environ.get("CLOUDFLOW_ROOT", "") or DEFAULT_BASE).resolve()
    root = base / "streamly_hardened"

    if args.pull:
        pull(base)

    if not root.exists():
        print(f"FATAL: streamly_hardened/ not found under {base}. "
