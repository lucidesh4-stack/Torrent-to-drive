...
## 2026-05-31 — Architecture Refactor & Stability Hardening

### Backend Architecture
- **Blueprint Transition**: Split `app.py` into a clean factory with routes moved to `routes/` (auth, cloud, search, history).
- **Service Decoupling**: Split `services.py` into `cloud_service.py` and `search_service.py`.
- **Config Polyfill**: Implemented `_get_cfg` in `security.py` to handle both Dataclass and Flask Config objects, eliminating `AttributeError` crashes.
- **Global State**: Moved `TokenBucketRateLimiter` to `extensions.py` to allow seamless rate-limiting within Blueprints.
- **Session Management**: Extracted `current_client` logic to `auth_utils.py` to prevent circular imports.

### Security & Reliability Fixes
- **Strict Storage Guard**: Implemented a mandatory storage check in `/api/add`. Requests are now blocked with a `400 "Insufficient storage"` error if the torrent exceeds available space.
- **Error Leak Prevention**: Replaced all `str(exc)` responses in bulk operations with generic, safe error messages.
- **Exception Hardening**: Replaced broad `except Exception` blocks with specific `(ConnectionError, TimeoutError)` handlers across all provider routes.
- **Magnet Validation**: Relaxed `BTIH_RE` regex and `startswith` checks to support a wider range of valid magnet link formats.

### Dev Tooling
- **Smarter Deployment**: Updated `deploy.bat`, `check.bat`, and `build.bat` to automatically detect system Python or use a provided portable path.
- **Verification Suite**: Added `verify_flask.py` to perform a headless boot-check of the application.

### Breaking changes
- None. API contracts and frontend behavior remain identical.
