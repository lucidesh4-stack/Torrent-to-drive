...
def _extract_quality(title: str) -> str:
    ...
    if c:
        ...
        if ssrc:
            label = ssrc.group(1).upper().replace("WEBDL", "WEB-DL")
            parts.append(label)
...
async def _resolve_bitsearch_via_doh(timeout: float) -> str | None:
    ...
    try:
        ...
        client = httpx.AsyncClient(verify=ssl_ctx, timeout=timeout)
...
def _make_row(*, name: str, infohash: str, seeds: Any, leeches: Any,
    ...
    return {
        ...
        "category": "Other",
        "encoder": enc,
        "encoder_norm": _normalize_encoder(enc),
...
class SearchService:
    ...
    async def imdb_suggestions(self, q: str) -> list[dict[str, Any]]:
        ...
        try:
            client = await HttpClientManager.get_instance().get_client()
            response = await client.get(url, timeout=self.config.request_timeout_seconds)
            response.raise_for_status()
            data = response.json()
        except Exception:
    ...
    async def bitsearch(self, q: str, category: str, sort: str, order: str, page: int = 1, dedup: bool = True) -> dict[str, Any]:
        ...
        per_page = _to_int(pagination.get("perPage"), pagination.get("limit"), payload.get("perPage"), payload.get("limit"), default=50)
        ...
        return {
            "results": raw_results,
            "pagination": {
                "page": _to_int(pagination.get("page"), payload.get("page"), default=page),
                "perPage": per_page,
                "total": total,
    ...
    async def _run_provider(self, name: str, q: str) -> list[dict[str, Any]]:
        ...
        try:
            client = await HttpClientManager.get_instance().get_client()
            if name == "apibay":
                return await _fetch_apibay(client, q, timeout)
            if name == "torrents-csv":
                return await _fetch_torrents_csv(client, q, timeout)
            if name == "bitsearch":
                return await self._bitsearch_rows(q)
        except Exception as exc:
