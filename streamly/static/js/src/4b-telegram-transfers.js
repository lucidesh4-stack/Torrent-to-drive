(() => {
  ...
  window.renderQueue = function(data) {
    ...
    // 1. Render Active Transfers (Multi-Parallel Support)
        ...
        activeCard.innerHTML = activeItems.map(active => {
          ...
          <div style="margin-bottom: 8px; padding: 10px 14px; background: rgba(255,255,255,0.03); border: 1px solid rgba(255,255,255,0.08); border-radius: 8px; display: flex; flex-direction: column; gap: 6px;">
            <div style="display: flex; align-items: center; justify-content: space-between; gap: 10px;">
              ...
              ${ftask ? `<button class="tg-active-cancel-btn" data-task-id="${ftask}" onclick="window.cancelTelegramTransfer('${ftask}', event)" style="background: rgba(239,68,68,0.12); border: 1px solid rgba(239,68,68,0.25); color: #f87171; border-radius: 4px; cursor: pointer; padding: 3px 8px; font-size: 12px; font-weight: 600; flex-shrink: 0; line-height: 1; transition: all 0.2s;" title="Cancel Active Export" aria-label="Cancel Active Export">✕</button>` : ''}
            </div>
    ...
    // Wire up cancel events on table rows
    document.querySelectorAll(".tg-cancel-btn, #tgQueueBody button").forEach((btn) => {
      btn.onclick = (e) => {
        const tid = e.currentTarget.dataset.taskId;
        if (tid) window.cancelTelegramTransfer(tid, e);
      };
    });

    // Show/hide + wire the "Cancel All" button
  ...
  // Delegated cancel event listener on active transfers card
  if ($("tgActiveTransferCard")) {
    $("tgActiveTransferCard").addEventListener("click", (e) => {
      const btn = e.target.closest(".tg-active-cancel-btn");
      if (!btn) return;
      e.preventDefault();
      e.stopPropagation();
      const tid = btn.dataset.taskId;
      if (tid) {
        window.cancelTelegramTransfer(tid, e);
      }
    });
  }
